...
// plus the reporting metrics from Table 5 (coverage, completeness,
...
export const listOpportunities = () => Opportunity.findAll({ order: [['createdAt', 'DESC']] });
...
// beneficiary: the information failure is distribution, not supply.
export async function publishOpportunity(user, { kind, title, org, detail }) {
  if (!['scholarship', 'job', 'training', 'announcement'].includes(kind)) bad('Invalid opportunity type');
  ...
  return sequelize.transaction(async (t) => {
    ...
    const all = await Beneficiary.findAll({ attributes: ['id', 'email'], transaction: t });
    const icon = { scholarship: '🎓', job: '💼', training: '📚', announcement: '📣' }[kind];
    await Notification.bulkCreate(
      all.map((b) => ({ beneficiaryId: b.id, icon, message: `New ${kind}: ${title.trim()}` })),
      { transaction: t },
    );
    await audit(t, { actorId: user.id, actorName: user.fullName, action: `Published ${kind}: ${title.trim()}`, entity: `Opportunity:${o.id}` });
    // Email is best-effort and must not fail the publication.
    Promise.all(all.filter((b) => b.email).map((b) => sendOpportunity({ to: b.email, title: title.trim(), org })))
      .catch((e) => console.error('[opportunity email]', e.message));
    return o;
  });
}
...
export const markRead = (user, id) =>
  Notification.update({ read: true }, { where: { id, beneficiaryId: user.beneficiaryId } });

// ══════════════════════════════════════════════════════════════
// REPORTS — Table 5 measures. ESTIMATED_POPULATION is the district
// estimate the coverage figure is measured against (Table 3: 2,400).
// ══════════════════════════════════════════════════════════════
const ESTIMATED_POPULATION = Number(process.env.ESTIMATED_PWD_POPULATION || 2400);
const DAY = 24 * 60 * 60 * 1000;

export async function reports() {
  const [beneficiaries, requests, impairments] = await Promise.all([
    Beneficiary.findAll({ attributes: ['id', 'sector', 'dailyChallenges', 'supportNeeds', 'nationalId', 'fullName', 'verified'] }),
    SupportRequest.findAll({ attributes: ['status', 'decisionReason', 'createdAt', 'completedAt'] }),
  ...
  // Duplication rate: records sharing a national ID or name+sector
  const seen = new Map();
  let duplicates = 0;
  for (const b of beneficiaries) {
    const key = b.nationalId || `${b.fullName}|${b.sector}`;
  ...
  const bySector = {};
  for (const b of beneficiaries) bySector[b.sector] = (bySector[b.sector] || 0) + 1;
  ...
  return {
    ...
    supportDelivered: byStatus.COMPLETED || 0,
...
export const auditLog = () => AuditLog.findAll({ order: [['createdAt', 'DESC']], limit: 200 });
